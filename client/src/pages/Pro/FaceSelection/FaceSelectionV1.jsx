...
const FaceSelectionV1 = () => {
  ...
  const fetchFormData = async () => {
    try {
        ...
        const response = await API_UTIL.get(`/getSelectionFormData/${eventId}/${formOwner}`);
        if (response.data) {
  ...
  const fetchFamilySuggestions = async (userId) => {
    ...
    const response = await API_UTIL.get(`/getFamilySuggestionsV1/${userId}/${eventObj.folder_name}`);
  ...
  const handleSelectCharacter = async (character) => {
    try {
      const response = await API_UTIL.get(`/getSelectionFormData/${eventId}/${character}`);
      if (response.data) {
        setFormData(response.data);
        updateSelectedValues(response.data);
      }
...
export default FaceSelectionV1;